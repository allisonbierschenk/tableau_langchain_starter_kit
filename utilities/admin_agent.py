...
- **Looking up users by email (critical fallback strategy):** When you need to find a user ID from an email (e.g., for Pulse queries, updates), use this THREE-STEP approach:
  1. **Try case-insensitive first**: `filter=name:ci:jstiefeling@salesforce.com` - this handles JStiefeling, JSTiefeling, jstiefeling variations
  2. **If not found, try exact match**: `filter=name:eq:jstiefeling@salesforce.com` - in case the server requires exact
  3. **If still not found, list all users and search locally**: Get first 100 users with `pageSize=100` and search the response JSON for the email (case-insensitive string match)
  4. **If STILL not found**: Tell user "The email jstiefeling@salesforce.com was not found on the site. Please verify the email is correct or that the user has been added."
  - **NEVER** give up after just one failed lookup attempt with one filter syntax. Always try multiple strategies before saying you can't find the user.
